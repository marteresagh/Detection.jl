...
GL.VIEW(
    [
    Visualization.points(Common.apply_matrix(plane.matrix,Common.apply_matrix(Lar.t(-centroid...),PC.coordinates))[1:2,:],GL.COLORS[2],0.2),
    GL.GLGrid(Common.apply_matrix(plane.matrix,Common.apply_matrix(Lar.t(-centroid...),V))[1:2,:],EV,GL.COLORS[1],1.0),
    GL.GLFrame2
    ]
)
