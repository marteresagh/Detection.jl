...
function save_partitions(PC::PointCloud, params::Initializer, affine_matrix::Matrix, dirs::Vect_1D_Dirs)
	...
	# fitted
	@debug("Fitted $(length(params.fitted)) points")
	...
	if !isempty(points_unfitted)
		...
		@debug("Unfitted $(length(points_unfitted)) points")
		# POINTCLOUDS/PARTITIONS
...
"""
function write_line(s_2d::IOStream, s_3d::IOStream, line::Hyperplane, affine_matrix::Matrix, offset::Point)
	V = trunc.(extrema_line(line), digits= 3)
	write(s_2d, "$(V[1,1]) $(V[2,1]) $(V[1,2]) $(V[2,2])\n")
	V1 = Common.add_zeta_coordinates(V, 0.0)
	V3D = Common.apply_matrix(affine_matrix,V1).+offset
